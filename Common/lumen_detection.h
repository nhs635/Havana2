...
public:
	explicit LumenDetection(int _outer_sheath, int _inner_offset, bool _data_save = false,
		...
		int _gw_th = 50, float _gwp_th = -0.4f, int _max_w_th = 160) :
...
public:
	...
	void preprocessing(np::Uint8Array2& src)
	{
		...
		GaussianBlur(input, output, cv::Size(7, 7), 3.5);
		...
		// Set size
		nradius = preprop.size(0);
		nalines = preprop.size(1);
	...
	void guide_wire_detection()
	{
		...
		for (int i = 4; i < 2 * nalines - 4; i++)
		{
			ippsSum_32f(&sum_profile(i - 1), 9, &sum_profile1(i), ippAlgHintAccurate);
			sum_profile1(i) = sum_profile1(i) / 9;
		}
		...
		for (int i = 0; i < peaks.size(); i++)
		{
			...
			if ((peaks.at(i) >= nalines / 2) && (peaks.at(i) < 3 * nalines / 2))
			{
				...
				if ((fwhm < 40) && ((sum_diff2(peaks.at(i)) < gwp_th) || (sum_diff2(peaks.at(i) - 1) < gwp_th) || (sum_diff2(peaks.at(i) - 2) < gwp_th)))
