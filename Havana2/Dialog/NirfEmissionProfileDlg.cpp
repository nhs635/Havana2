...
#else
	if (m_bIsStreaming)
		m_pScope = new QScope2({ 0, (double)m_pConfig->nAlines }, { m_pConfig->nirfRange.min, m_pConfig->nirfRange.max }, 2, 2, 1, 1, 0, 0, "", "", true);
	else
		m_pScope = new QScope2({ 0, (double)m_pResultTab->m_nirfMap1.size(0) }, { m_pConfig->nirfRange.min, m_pConfig->nirfRange.max });
#endif
